...
const fetch = require('node-fetch')
    ...
    async(Void, citel, text) => {
        ...
        google({ 'query': text}).then(res => {
            let msg= `*🌙⃝🧚‍♀️عـنـوان الـبـحـث┇* ${text} \n\n`;
            for (let g of res) {
                msg+= `*🌙⃝🧚‍♀️الـعـنـوان┇* ${g.title}\n`;
                msg+= `*🌙⃝🧚‍♀️الـوصـف┇* ${g.snippet}\n`;
                msg+= `*🌙⃝🧚‍♀️الـرابـط┇* : ${g.link}\n\n*❋ ─═══━•┇🌙┇•━═══─ ❋*\n\n`;
            }
        ...
        for (let i = 0; i < randomxx; i++) {
            ...
            try {
                ...
                if (anu1 == '401' || anu1.status.length == 0) {
                    ...
                    text += `*🌙⃝🧚‍♀️الـرقـم📱┇* wa.me/${anu[0].jid.split("@")[0]}\n *🌙⃝🧚‍♀️الـبـايـو✨┇* ${anu1.status}\n*🌙⃝🧚‍♀️الـتـاريـخ🍁┇* ${moment(anu1.setAt).tz('Asia/Kolkata').format('HH:mm:ss DD/MM/YYYY')}\n\n`
                }
            } catch {
